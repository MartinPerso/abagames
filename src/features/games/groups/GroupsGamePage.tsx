...
type TargetCelebrationMotion = {
  ...
  animationName: 'groups-target-voyage-arc' | 'groups-target-voyage-zigzag' | 'groups-target-voyage-spiral'
...
function createTargetCelebrationMotion(): TargetCelebrationMotion {
  const animationNames: TargetCelebrationMotion['animationName'][] = [
    'groups-target-voyage-arc',
    'groups-target-voyage-zigzag',
    'groups-target-voyage-spiral',
  ]
...
export function GroupsGamePage() {
  const [searchParams] = useSearchParams()
  const language = parseLanguageParam(searchParams.get('lang'))
  const groupCount = getStoredGroupsCount()
  const maxObjects = getStoredGroupsMaxObjects()
  const minGap = getStoredGroupsMinGap()
  const itemMode = getStoredGroupsItemMode()
  const answerPointerEnabled = getStoredGroupsAnswerPointerEnabled()
  ...
  const text = groupsGameTextByLanguage[language]
  const itemLabels = itemLabelByLanguage[language]
  const superRewardText = superRewardUiTextByLanguage[language]

  const [roundIndex, setRoundIndex] = useState(0)
  const [round, setRound] = useState(() => createRound(0, { groupCount, maxObjects, minGap, itemMode }))
  ...
  function moveToNextRound() {
    ...
    setRound(createRound(nextIndex, { groupCount, maxObjects, minGap, itemMode }))
    setFeedback('idle')
  ...
  return (
    <main className="app-shell groups-page">
      <header className="groups-header">
        <h1>{groupsGameNameByLanguage[language]}</h1>
      ...
      <section
        className={`choice-grid is-groups-${round.choices.length} ${feedback === 'wrong' ? 'is-wrong' : ''} ${animateChoiceReveal ? 'is-revealing' : ''}`}
      >
